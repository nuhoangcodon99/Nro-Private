package red.s1.boss;

import red.team.server.Manager;
import red.utils.Logger;

public class BossID {

    private BossID() {

    }

    public static final int KUKU = -20;
    public static final int MAP_DAU_DINH = -21;
    public static final int RAMBO = -22;
     public static final int NRONAMEC = -32252;
    //
    public static final int SO_4 = -23;
    public static final int SO_3 = -24;
    public static final int SO_2 = -25;
    public static final int SO_1 = -26;
    public static final int TIEU_DOI_TRUONG = -27;

    public static final int TDST = -223;
    public static final int MABU_13H = 22342;

    public static final int FIDE = -28;

    public static final int COOLER = -2001;
    public static final int COOLER_1 = -2002;
    public static final int THAN_MEO_KARIN =-8686;
        public static final int YAJIROBE_KARIN =-8666;
    public static final int COOLER_2 = -2003;
    public static final int FROST = -2004;
    public static final int FROST_1 = -2005;
    public static final int FROST_2 = -2005;
    public static final int FROST_3 = -2005;
    public static final int CHILL = -2006;
    public static final int CHILL_1 = -2007;
    public static final int CHILL_2 = -2008;

    public static final int ANDROID_19 = -30;
    public static final int DR_KORE = -31;

    public static final int ANDROID_13 = -32;
    public static final int ANDROID_14 = -33;
    public static final int ANDROID_15 = -34;

    public static final int PIC = -35;
    public static final int POC = -36;
    public static final int KING_KONG = -37;

    public static final int XEN_BO_HUNG = -100;
    public static final int SIEU_BO_HUNG = -101;
    public static final int XEN_CON_1 = -102;


    public static final int COOLER_GOLD = -201;
    public static final int VUA_COLD = -202;

    public static final int HACHIYACK = -207;
    public static final int DR_LYCHEE = -208;

    public static final int DORAEMON = -222;
    public static final int NOBITA = -212;
    public static final int XUKA = -220;
    public static final int CHAIEN = -219;
    public static final int XEKO = -218;
    public static final int DORAEMON_TEAM = -2300;

    public static final int SOI_GIANGHO = -1000;
    public static final int ODO_GIANGHO = -1001;
    public static final int XINBATO_GIANGHO = -1002;
    public static final int CHACHA_GIANGHO = -1003;
    public static final int PONPUT_GIANGHO = -1004;
    public static final int CHANXU_GIANGHO = -1005;
    public static final int TAUPAYPAY_GIANGHO = -1006;
    public static final int YAMCHA_GIANGHO = -1007;
    public static final int JACKYCHUN_GIANGHO = -1008;
    public static final int THIENXINHANG_GIANGHO = -1009;
    public static final int GIANGHO = -1010;

     public static final int Vodaihatmit = -1011;

    public static final int MABU = -214;
    public static final int CUMBER = -215;
    public static final int BLACK3 = -231;
    public static final int BROLY = -1;
    public static final int BROLY1 = -2;
    public static final int BROLY2 = -3;
    public static final int BROLY3 = -4;
    public static final int SONGOKU_TA_AC = -217;
    public static final int FIDE_ROBOT = -216;

    // super 17 saga
    public static final int SUPER_ANDROID_17 = -2470;
    public static final int DR_MYUU = -2471;
    public static final int DR_KORE_GT = -2472;

    public static final int Rong_1Sao = -224;
    public static final int Rong_2Sao = -225;
    public static final int Rong_3Sao = -226;
    public static final int Rong_4Sao = -227;
    public static final int Rong_5Sao = -228;
    public static final int Rong_6Sao = -229;
    public static final int Rong_7Sao = -230;

    // mabu 12h

    public static final int DRABURA = -233;
    public static final int BUI_BUI = -234;
    public static final int YA_CON = -235;
    public static final int MABU_12H = -236;
    public static final int DRABURA_2 = -237;
    public static final int BUI_BUI_2 = -238;

    // ngokhi
    public static final int DUONGTANG = -3000;
    public static final int NGOKHONG = -3001;
    public static final int CHUBATGIOI = -3002;
     public static final int XA_TANG = -3003;

    // bojack
    public static final int BOJACK_GIANGHO = -4000;
    public static final int BIDO_GIANGHO = -4001;
    public static final int ZANGYA_GIANGHO = -4002;
    public static final int KOGU_GIANGHO = -4003;
    public static final int BUJIN_GIANGHO = -4004;
    public static final int BOJACK_TEAM = -4004;
    // luffy
    public static final int LUFFY_GIANGHO = -5000;
    public static final int ZORO_GIANGHO = -5001;
    public static final int USOP_GIANGHO = -5002;
    public static final int NAMI_GIANGHO = -5003;
    public static final int ROBIN_GIANGHO = -5004;
    public static final int SANJI_GIANGHO = -5005;
    public static final int FRANKY_GIANGHO = -5006;
    public static final int CHOPPER_GIANGHO = -5007;
    public static final int BROOK_GIANGHO = -5008;
    public static final int LUFFY_TEAM = -5008;
    // tanjiro
    public static final int TANJIRO_GIANGHO = -6000;
    public static final int NEZUKO_GIANGHO = -6001;
    public static final int INOSUKE_GIANGHO = -6002;
    public static final int ZENITSU_GIANGHO = -6003;
    public static final int SANQUY_TEAM = -6003;
    // Boss trung
    public static final int MABU_THAN = -7001;
    public static final int BERUS_THAN = -7002;
    public static final int ADMIN_THAN = -7003;
    public static final int NGOKHONG_THAN = -7004;
    // granola
    public static final int GRANOLA = -8003;
    //
    public static final int BU_BUNG = -22053;

    // women
    public static final int CHICHI = -9000;
    public static final int BULMA = -9001;
    public static final int LYTIEUNUONG = -9002;
    public static final int SO18 = -9003;
    public static final int THODAICA = -9004;
    //Vo dai hat mit
    public static final int DRACULA = -10000;
    public static final int NGUOIVOHINH = -10001;
    public static final int BONGBANG = -10002;
    public static final int SATAN = -10003; 
    public static final int THODAUBAC = -10004;
    //VU TRU 6
    public static final int SUPER_CAULIFA = -11000;
    public static final int CAULIFA_1 = -11001;
    public static final int CAULIFA = -11002;

    //chill

    public static final int ANDROID_21 = -11005;
    //BASIL
    public static final int BASIL = -12000;
    public static final int LAVENDE = -12001;
    public static final int BERGAMO = -12002;
    public static final int BACONSOI = -12003;
    //GOKU,PICOLO,CADIC
    public static final int GOKU_FAKE = -13000;
    public static final int POCOLO_FAKE = -13001;
    public static final int CADIC_FAKE = -13002;
    //PILAP
    public static final int PILAP = -14000;
    public static final int MAI = -14001;
    public static final int SU = -14002;
    //than linh
    public static final int THAN_VU_TRU = -16000;
    public static final int THAN_MEO = -16001;
    public static final int THUONG_DE = -16002;
    //blue
    public static final int GOKU_BLUE = -17000;
    public static final int CADIC_BLUE = -17001;
    //
    public static final int ZENO_VANG = -18001;
    public static final int ZENO_XANH = -18000;
    public static final int ZENO_TIM = -18002;
    public static final int ZENO= -18004;
    //
    public static final int UUB= -19000;
    //
    public static final int BROLY_BASE= -20000;
    public static final int BROLY_BASE_1= -20001;
    public static final int BROLY_BASE_2= -20002;
    public static final int BROLY_BASE_3= -20003;
    //
    public static final int DUONG_TANG_HOTONG= -300003;
    public static final int MI_NUONG_HOTONG= -300004;
    public static final int KI_LAN_HOTONG= -300004;
    //DOANH TRAI
    public static final int TRUNG_UY_TRANG = -30000;
    public static final int TRUNG_UY_XANH_LO = -30001;
    public static final int TRUNG_UY_THEP = -30002;
    public static final int NINJA_AO_TIM = -30003;

    public static final int NINJA_AO_TIM_CLONE = -30004;
    public static final int NINJA_AO_TIM_CLONE_MAX = -30005;// max 20 con ngẫu nhiên

    public static final int ROBOT_VE_SI1 = -30006;
    public static final int ROBOT_VE_SI2 = -30007;
    public static final int ROBOT_VE_SI3 = -30008;
    public static final int ROBOT_VE_SI4 = -30009;
    public static final int NINJA_AO_TIM1 = -30010;
    public static final int NINJA_AO_TIM2 = -30011;
    public static final int NINJA_AO_TIM3 = -30012;
    public static final int NINJA_AO_TIM4 = -30013;
    //
    public static final int SON_TINH = -40000;
    public static final int THUY_TINH = -40001;
    //

    public static final int VOI_9_NGA = -40002;
    public static final int GA_9_CUA = -40003;
    public static final int NGUA_9_LMAO = -40004;

    //
    public static final int TAU_PAY_PAY_ROBOT = -40005;
    public static final int TAU_PAY_PAY_KARIN = -40006;
    //
    public static final int FIDE_DIA_NGUC = -41000;
    public static final int XEN_DIA_NGUC = -41001;
    public static final int PAIKUHAN_DIA_NGUC = -41002;
    public static final int DEMON_KING_DIA_NGUC = -41003;
    //

    public static final int BLACK = -203;
    public static final int BLACK1 = -241;
    public static final int BLACK2 = -242;

    public static final int ZAMASMAX = -243;
    public static final int ZAMASZIN = -244;

    public static final int THAN_HUY_DIET = -239;
    public static final int THIEN_SU_WHIS = -240;
    public static final int THAN_HUY_DIET_CHAMPA = -245;
    public static final int THIEN_SU_VADOS = -246;
    public static final int SUPER_XEN = -248;
    public static final int KAMIRIN = -253;
    public static final int KAMILOC = -249;
    public static final int KAMI_SOOME = -250;
    public static final int CUMBERYELLOW = -251;
    public static final int CUMBERBLACK = -252;
    public static final int CUMBERRED = -25300;
    public static final int LUCYONFIRE = -25301;
    public static final int KABUTO = -25302;
    public static final int KHI_BAC = -25305;
    public static final int KHI_DEN = -25306;
    public static final int KHI_VANG = -25307;
    public static final int NHAT_THAN = -25308;
    public static final int NGUYET_THAN = -25309;

    public static final byte SOI_HEC_QUYN = -77;
    public static final byte O_DO = -78;
    public static final byte XINBATO = -79;
    public static final byte CHA_PA = -80;
    public static final byte PON_PUT = -81;
    public static final byte CHAN_XU = -82;
    public static final byte TAU_PAY_PAY = -83;
    public static final byte YAMCHA = -84;
    public static final byte JACKY_CHUN = -85;
    public static final byte THIEN_XIN_HANG = -86;
    public static final byte LIU_LIU = -87;
    public static final byte THIEN_XIN_HANG_CLONE = -88;
    public static final byte THIEN_XIN_HANG_CLONE1 = -89;
    public static final byte THIEN_XIN_HANG_CLONE2 = -90;
    public static final byte THIEN_XIN_HANG_CLONE3 = -91;

    public static int getCountBosses() {
        // Count the number of constants using reflection
        int count = 0;
        try {
            Class<BossID> bossClass = BossID.class;
            count = bossClass.getFields().length;
        } catch (Exception e) {
            e.printStackTrace();
            Logger.logException(Manager.class, e, "Lỗi count boss");
        }
        return count;
    }

}
