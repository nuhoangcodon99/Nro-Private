...
public class Trade {
    ...
    public void addItemTrade(Player pl, byte index, int quantity) {
        if (pl.getSession().actived) {
            ...
            Service.getInstance().sendThongBao(pl,
                    "VUI LÒNG KÍCH HOẠT TÀI KHOẢN Liên Hệ Admin ĐỂ MỞ KHÓA TÍNH NĂNG GIAO DỊCH");
            removeItemTrade(pl, index);
    ...
    private boolean isItemCannotTran(Item item) {
        for (Item.ItemOption io : item.itemOptions) {
            if (io.optionTemplate.id == 30 || io.optionTemplate.id == 34 || io.optionTemplate.id == 35
        ...
        switch (item.template.type) {
            case 27: //
                if (item.template.id != 457 && item.template.id != 590 && item.template.id==2077 && item.template.id==921 && item.template.id==2074 && item.template.id==2075 && item.template.id==2076 && item.template.id==2036 && item.template.id==933 && item.template.id==934) {
                    return true;
                } else {
                    return false;
                }
    ...
    public void lockTran(Player pl) {
        ...
        try {
            ...
            if (pl.getSession().version > 214 && pl.getSession().version <= 231) {
                ...
                } else if (!player1.getSession().actived) {
                    TransactionService.gI().cancelTrade(pl);
                } else if (!player2.getSession().actived) {
                    TransactionService.gI().cancelTrade(pl);
                } else {
        ...
        if (tradeStatus != SUCCESS) {
            ...
            if (tradeStatus != SUCCESS) {
                ...
                if (tradeStatus == SUCCESS) {
                    ...
                    // Client.gI().kickSession(player1.getSession());
                    // Client.gI().kickSession(player2.getSession());
                }
                sendNotifyTrade(tradeStatus);
            }
        }
