            ...
            if (pl != null) {
                if (player.isAdmin() || player.nPoint.teleport) {
                    if (!pl.itemTime.isUseAnDanh || player.isAdmin()) {
                        if ((player.isAdmin() || !pl.zone.isFullPlayer()) && !MapService.gI().isMapDoanhTrai(pl.zone.map.mapId)&& !MapService.gI().isMapMiNuong(pl.zone.map.mapId)&& !MapService.gI().isMapBanDoKhoBau(pl.zone.map.mapId)&& !MapService.gI().isMapKhiGaHuyDiet(pl.zone.map.mapId)) {
